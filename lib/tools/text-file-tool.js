...
export function createTextFileTool({ getCwd, getSessionPath, registerSessionFile } = {}) {
  return {
    ...
    label: "text_file",
    description: "Read, write, replace, insert, delete, and batch-edit plain text files. This is the PRIMARY tool for all text file operations — prefer it over bash/Python/Node scripts, the edit tool, and the write tool. Supports regex, case-insensitive matching, line-based operations, and transactional batches.",
    promptSnippet: "PRIMARY text file tool — use this (not bash/Python) for reading, writing, replacing, inserting, deleting, or batch-editing text files",
    promptGuidelines: [
