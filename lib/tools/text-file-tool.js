...
export function createTextFileTool({ getCwd, getSessionPath, registerSessionFile } = {}) {
  return {
    ...
    promptSnippet: "Inspect and modify plain text files — read, create, write, append, replace, insert, delete, batch (prefer this over bash/Python for text edits)",
    promptGuidelines: [
      "Prefer text_file over bash/Python/Node for ANY text file operation — it is deterministic, faster, and handles encoding/backup correctly",
      "Do NOT write Python/Node scripts for text search-and-replace; use text_file with action=replace or action=insert instead",
      "For multi-step text edits, use text_file batch action instead of chaining multiple bash commands",
      "text_file supports regex-based replace/insert, case-insensitive matching, and line-based operations — no scripting needed",
    ],
