...
- **如果上方提供了今日对话摘要，说明用户今天有对话。绝对禁止写「今天没有说话」「全天无对话」「工作区静默」「无直接对话」等否定性结论。** 这不是风格选择问题，是事实错误。
- 后台活动（巡检/定时任务）不是对话。不要把巡检中「用户无新活动」的表述误解成「今天没有对话」——巡检记录的只是两次巡检间隔之间有无活动，不代表全天情况。
- 先读对话摘要，再读后台活动。对话＞后台活动。`;
  }
...
export async function writeDiary(opts) {
  ...
  if (hasConvoMaterials && noConvoPatterns.some(p => p.test(finalContent))) {
    const warnMsg = isZh
      ? "日记生成警告：检测到「无对话」表述与素材矛盾（存在对话素材但 LLM 误判为静默日）。请人工复核。"
      : "Diary generation warning: detected 'no conversation' claim contradicting available materials. Please review manually.";
    log.warn(warnMsg);
