...
/**
 * Decide whether a phone session should be compacted based on token usage
 * relative to the model's context window.
 *
 * - Idle sessions: compact when tokens exceed ~70% of context window.
 * - Active sessions: compact when tokens exceed ~85% of context window (hard).
 * - Otherwise: return null (no compaction needed).
 */
export function shouldCompactAgentPhoneSession({
  ...
  if (!isActive && ratio >= idleRatio) return "idle";
  if (isActive && ratio >= hardRatio) return "hard";
  return null;
}
