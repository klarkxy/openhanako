...
const DEFAULT_MAX_TOOL_RESULT_BYTES = 128 * 1024; // 128KB ≈ 32K token（修复：32KB 截断导致 read→edit 信息断层）
const DEFAULT_HARD_TRUNCATE_THRESHOLD = 0.85;    // messagesToSummarize 超 85% 窗口 → 硬截断

// 不截断的只读工具列表：这些工具的输出是 edit 等其他工具的前置输入，
// 截断会直接导致后续 edit 的 oldText 匹配失败。
const READ_ONLY_TOOLS_EXEMPT = new Set(["read", "grep", "find", "ls"]);
  ...
  return function (pi) {
    ...
    pi.on("tool_result", (event) => {
      try {
        ...
        // 只读工具（read/grep/find/ls）不截断，因为其输出是 edit 的前置输入
        // 截断 read 会导致后续 edit 的 oldText 匹配失败，严重影响工具成功率
        if (READ_ONLY_TOOLS_EXEMPT.has(event.toolName)) return undefined;
