...
const XLSX_FULL_ROW_LIMIT = 100;
const XLSX_FULL_COLUMN_LIMIT = 30;
const XLSX_FULL_CELL_LIMIT = 3000;
const XLSX_PREVIEW_ROW_LIMIT = 25;
const XLSX_PREVIEW_COLUMN_LIMIT = 12;
...
function shouldSummarizeSheet(sheet) {
  ...
  return (
    rows > XLSX_FULL_ROW_LIMIT ||
    columns > XLSX_FULL_COLUMN_LIMIT ||
    rows * columns > XLSX_FULL_CELL_LIMIT
  );
}

function collectXlsxRows(sheet, maxRows, maxColumns) {
  const rows = [];
  for (let r = 1; r <= maxRows; r += 1) {
    ...
    for (let c = 1; c <= maxColumns; c += 1) {
      cells.push(normalizeXlsxCellText(row.getCell(c)));
    }
    rows.push(cells);
  }
  return rows;
...
function formatXlsxRows(rows) {
  if (rows.length === 0) return [];

  const colWidths = [];
  for (let c = 0; c < (rows[0]?.length || 0); c += 1) {
    let max = 0;
    for (const row of rows) {
      const len = (row[c] || "").length;
      if (len > max) max = len;
    }
    colWidths.push(Math.min(max, XLSX_DISPLAY_CELL_WIDTH));
  }
...
async function xlsxToText(filePath) {
  ...
  for (const sheet of sheets) {
    ...
    if (shouldSummarizeSheet(sheet)) {
      const previewRows = Math.min(sheet.rowCount, XLSX_PREVIEW_ROW_LIMIT);
      const previewColumns = Math.min(sheet.columnCount, XLSX_PREVIEW_COLUMN_LIMIT);
      parts.push(`Rows: ${sheet.rowCount}`);
      parts.push(`Columns: ${sheet.columnCount}`);
      parts.push(`Preview: first ${previewRows} row(s) and ${previewColumns} column(s)`);
      parts.push(...formatXlsxRows(collectXlsxRows(sheet, previewRows, previewColumns)));
      parts.push(
        `[truncated: showing ${previewRows}/${sheet.rowCount} rows and ${previewColumns}/${sheet.columnCount} columns]`,
      );
    } else {
      parts.push(...formatXlsxRows(collectXlsxRows(sheet, sheet.rowCount, sheet.columnCount)));
    }
    parts.push("");
  }
