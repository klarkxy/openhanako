 ...
 *   3. Python 命令在沙盒开启时自动走工作区 venv（.hana-python/），
 *      首次调用由系统 Python 通过 `python -m venv --copies` 引导创建；
 *      pip cache 共享到 hanakoHome/.ephemeral/pip-cache，跨 workspace 复用。
 *   4. POSIX shell 命令走 bash/ash/sh 兼容层
 *
...
// ── 工作区 Python 虚拟环境 ──────────────────────────
//
// 沙盒开启时，Python 命令自动路由到工作区内的 .hana-python/ venv。
// 首次调用由系统 Python 引导创建（python -m venv --copies），后续零开销。
// pip 下载缓存共享到 hanakoHome/.ephemeral/pip-cache，跨 workspace 复用。
// 引导失败时降级为系统 Python（带警告日志），不会阻断 agent 工作流。
const WORKSPACE_VENV_DIR = ".hana-python";

/**
 * 从 python.exe 路径向上查找 Python 安装根目录（包含 Lib/ 的那层）
 * @param {string} executable  python.exe 的绝对路径
 * @returns {string|null}
 */
function findPythonInstallRoot(executable) {
  if (!executable) return null;
  const dir = dirnameRuntimePath(executable);
  if (existsSync(joinRuntimePath(dir, "Lib"))) return dir;
  const parent = dirnameRuntimePath(dir);
  if (existsSync(joinRuntimePath(parent, "Lib"))) return parent;
  return dir;
}

/** 工作区 venv 的 python.exe 完整路径 */
function resolveWorkspaceVenvPython(cwd) {
  return joinRuntimePath(cwd, WORKSPACE_VENV_DIR, "Scripts", "python.exe");
}

/** 工作区内 pip 缓存（仅 hanakoHome 不可用时作为兜底） */
function resolveWorkspaceVenvPipCache(cwd) {
  return joinRuntimePath(cwd, WORKSPACE_VENV_DIR, "pip-cache");
}

/**
 * 在工作区内创建 Python 虚拟环境
 *
 * 通过沙盒 helper 执行 `python -m venv --copies <cwd>/.hana-python`。
 * --copies 确保文件复制而非符号链接（Windows restricted-token 下 symlink 不可靠）。
 * 超时 120s，覆盖大型 stdlib（如 Python 3.11+ 约 200MB）的复制。
 *
 * @throws {Error} 创建成功但 python.exe 未出现在预期路径时抛出
 */
async function bootstrapWorkspaceVenv({ systemPythonExe, cwd, sandbox, shellEnv }) {
  const venvPath = joinRuntimePath(cwd, WORKSPACE_VENV_DIR);
  log.log(`[win32-exec] Bootstrapping workspace venv at ${venvPath} using ${systemPythonExe}`);
  await spawnViaSandboxHelper({
    sandbox,
    executable: systemPythonExe,
    args: ["-m", "venv", "--copies", venvPath],
    cwd,
    env: shellEnv,
    onData: () => {},
    signal: undefined,
  ...
  if (!existsSync(resolveWorkspaceVenvPython(cwd))) {
    throw new Error(
      `[win32-exec] Workspace venv bootstrap succeeded but python.exe not found at expected path: ${resolveWorkspaceVenvPython(cwd)}`
    );
  }
  log.log(`[win32-exec] Workspace venv ready at ${venvPath}`);
}

/**
 * 共享 pip 缓存路径
 *
 * 指向 hanakoHome/.ephemeral/pip-cache（该目录已在沙盒可写白名单中）。
 * 所有 workspace 共用同一缓存，避免重复下载 numpy/pandas 等大型包。
 * @param {string|null} hanakoHome
 * @returns {string|null}
 */
function resolveSharedPipCache(hanakoHome) {
  return hanakoHome ? joinRuntimePath(hanakoHome, ".ephemeral", "pip-cache") : null;
}

/**
 * 构建 venv Python 进程的环境变量
 *
 * - PATH 头部追加 venv/Scripts/，使 pip / 入口点可直接解析
 * - PIP_CACHE_DIR → 优先共享缓存，不可用时退回工作区内缓存
 * - PYTHONUNBUFFERED=1 → 实时输出，agent 无需等待缓冲区刷新
 */
function buildVenvPythonEnv(baseEnv, cwd, { hanakoHome } = {}) {
  const env = cleanShellEnv(baseEnv || {});
  const pathKey = Object.keys(env).find((k) => k.toLowerCase() === "path") ?? "PATH";
  // Prepend venv Scripts dir so pip / entry points resolve
  const venvScripts = joinRuntimePath(cwd, WORKSPACE_VENV_DIR, "Scripts");
  const current = String(env[pathKey] ?? "");
  const delimiter = current.includes(";") || isWin32PathLike(venvScripts) ? ";" : path.delimiter;
  env[pathKey] = [venvScripts, ...current.split(delimiter).filter(Boolean)].join(delimiter);
  // 共享 pip cache 到 hanakoHome/.ephemeral/pip-cache（沙盒可写，跨 workspace 复用）
  // 若 hanakoHome 不可用则退回到工作区内缓存
  const sharedCache = resolveSharedPipCache(hanakoHome);
  env.PIP_CACHE_DIR = sharedCache || resolveWorkspaceVenvPipCache(cwd);
...
export function createWin32Exec({ sandbox = null } = {}) {
  return async (command, cwd, { onData, signal, timeout, env }) => {
    ...
    if (route.runner === "python") {
      ...
      // ── 沙盒开启 → 自动走工作区虚拟环境 ──
      if (sandboxIsEnabled(sandbox)) {
        const wsVenvPython = resolveWorkspaceVenvPython(cwd);

        if (existsSync(wsVenvPython)) {
          // 工作区 venv 已就绪，直接使用
          const venvEnv = buildVenvPythonEnv(shellEnv, cwd, { hanakoHome: sandbox?.hanakoHome });
          return spawnViaSandboxHelper({
            sandbox,
            executable: wsVenvPython,
        ...
        // 无工作区 venv → 用系统 Python 引导创建
        const pythonEnv = getRuntimeEnvForCandidate(shellEnv, pythonInfo);
        try {
          await bootstrapWorkspaceVenv({
            systemPythonExe: pythonInfo.executable,
            cwd,
            sandbox,
            shellEnv: pythonEnv,
          });
        } catch (bootstrapErr) {
          // 引导失败 → 降级使用系统 Python（带警告日志）
          log.warn(
            `[win32-exec] Workspace venv bootstrap failed: ${bootstrapErr.message}. ` +
            `Falling back to system Python (pip install 将无法写入系统 site-packages).`
          );
          return spawnViaSandboxHelper({
            sandbox,
