...
import { collectTextFilePathChecks, TEXT_FILE_TOOL_NAME } from "../tools/text-file-tool.js";
...
export function wrapTextFileTool(tool, guard, cwd, opts = {}) {
  return {
    ...tool,
    execute: async (toolCallId, params, ...rest) => {
      if (opts.getSandboxEnabled && !opts.getSandboxEnabled()) {
      ...
      if (tool.name === TEXT_FILE_TOOL_NAME) {
        for (const check of collectTextFilePathChecks(params, cwd)) {
          const result = checkWithExternalReadGrant(guard, check.path, check.operation, opts);
          if (!result.allowed) {
            return blockedResult(t("sandbox.restrictedPath", { path: check.path }));
          }
        }
      }
