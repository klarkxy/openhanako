...
import { wrapPathTool, wrapBashTool, wrapTextFileTool } from "./tool-wrapper.js";
...
import { TEXT_FILE_TOOL_NAME } from "../tools/text-file-tool.js";
...
export function createSandboxedTools(cwd, customTools, {
  ...
  const wrappedCustomTools = customTools.map((tool) => (
    tool?.name === TEXT_FILE_TOOL_NAME
      ? wrapTextFileTool(tool, guard, cwd, wrapOpts)
      : tool
  ));
