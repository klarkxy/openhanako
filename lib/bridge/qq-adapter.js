...
/**
 * 去除发给 QQ 的消息中的内部标签块（mood/pulse/reflect/think/thinking/tool_code）。
 * 作为最终安全网，确保这些内部块绝不会泄漏到 QQ 聊天中。
 */
function cleanQqMessage(text) {
  if (!text) return text;
  let cleaned = text;
  // XML 格式标签
...
export const QQ_STREAMING_CAPABILITIES = createStreamingCapabilities({
  platform: "qq",
  mode: "batch_pulse",
  scopes: ["dm"],
  pulseIntervalMs: 20_000,
...
export function createQQAdapter({ appID, appSecret, agentId, onMessage, dmGuildMap, onDmGuildDiscovered, onStatus }) {
  ...
  return {
    ...
    streamingCapabilities: QQ_STREAMING_CAPABILITIES,

    async sendReply(chatId, text, replyContext = null) {
      const cleaned = cleanQqMessage(text);
      if (!cleaned) return;
      ...
      for (let i = 0; i < cleaned.length; i += MAX) {
        const chunk = cleaned.slice(i, i + MAX);
        let lastError = null;
        const errors = [];
        for (const endpoint of messageEndpoints) {
    ...
    async sendBlockReply(chatId, text, replyContext = null) {
      ...
      await this.sendReply(chatId, cleaned, replyContext);
