...
import { bridgeRelationLabel, normalizeBridgeRelation } from "./contacts/policy.js";

export const BRIDGE_NOTIFY_PLATFORMS = ["wechat", "feishu", "telegram", "qq", "onebot"];
...
export function buildBridgeContext(input = {}, locale = "zh") {
  ...
  const relation = normalizeBridgeRelation(input.relation || (role === "owner" ? "self" : "stranger"));
  ...
  const infoDisclosure = input.infoDisclosure || null;
  const toneProfile = input.toneProfile || null;
  ...
  return {
    ...
    role,
    relation,
    relationLabel: bridgeRelationLabel(relation, locale),
    sessionKey,
    agentId,
    userId,
    chatId,
...
export function buildBridgePromptLine(context) {
  if (!context?.isBridgeSession || !context.platform) return "";
  const label = bridgePlatformLabel(context.platform, "en");
  if (!label) return "";
...
export function appendBridgePromptLine(prompt, context) {
  const line = buildBridgePromptLine(context);
...
export function bridgeContextIndexMeta(context, meta = {}) {
  ...
  return {
    ...
    ...(context.infoDisclosure ? { infoDisclosure: context.infoDisclosure } : {}),
    ...(context.toneProfile ? { toneProfile: context.toneProfile } : {}),
  };
}
