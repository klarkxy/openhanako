...
export class BridgeManager {
  ...
  async _handleMessage(platform, msg) {
    ...
    let pending = this._pending.get(sessionKey);
    if (!pending) {
      pending = { kind: "dm-buffer", lines: [], attachments: [], platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner, bridgeAudience, agentId, messageThreadId, replyContext };
      this._pending.set(sessionKey, pending);
    }
    pending.lines.push(line);
    this._appendPendingAttachments(entry, pending.attachments, attachments);
    Object.assign(pending, { platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner, bridgeAudience, messageThreadId, replyContext });
  ...
  async _flushPending(sessionKey) {
    ...
    // 取出所有缓冲消息和附件
    const { lines, attachments: pendingAttachments = [], platform, chatId, senderName, avatarUrl, userId, isGroup, isOwner, bridgeAudience, agentId, messageThreadId, replyContext } = batch;
