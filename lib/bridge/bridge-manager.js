...
import { getEffectiveBridgeRelationPolicy } from "./contacts/policy.js";
import { resolveBridgeAudience } from "./contacts/store.js";
...
const ADAPTER_REGISTRY = {
  ...
  onebot: {
    create: (creds, onMessage, hooks, agentId) => createOneBotAdapter({
      apiBase: creds.apiBase,
      accessToken: creds.accessToken,
      selfId: creds.selfId,
      requireAtInGroup: creds.requireAtInGroup,
      agentId,
      onMessage,
      onStatus: hooks?.onStatus,
    }),
    getCredentials: (cfg) => {
      const apiBase = cfg?.apiBase || cfg?.apiUrl || cfg?.endpoint;
      return cfg?.enabled && apiBase
        ? {
            apiBase,
            accessToken: cfg.accessToken || "",
            selfId: cfg.selfId || "",
            requireAtInGroup: cfg.requireAtInGroup,
          }
        : null;
    },
    ownerSessionKey: (userId, agentId) => `ob_dm_${userId}@${agentId}`,
    connectsAsync: true,
  },
};
...
class StreamCleaner {
  ...
  feed(delta) {
    ...
    while (true) {
      if (this._inTag) {
        const closeRe = new RegExp(`<\\/${this._tagName}\\s*>`, "i");
        const cm = this._buf.match(closeRe);
        if (!cm || cm.index === undefined) break; // 等待更多数据
        const ci = cm.index;
        const closeLen = cm[0].length;
        this._buf = this._buf.slice(ci + closeLen).replace(/^\s*/, "");
        this._inTag = false;
        this._tagName = null;
      } else {
        // 寻找最近的开标签（case-insensitive，容忍标签名后空格）
        let best = null;
        let bestIdx = Infinity;
        let bestLen = 0;
        for (const tag of STRIP_TAGS) {
          const openRe = new RegExp(`<${tag}\\s*>`, "i");
          const m = this._buf.match(openRe);
        ...
        if (best) {
          ...
          this._buf = this._buf.slice(bestIdx + bestLen);
          this._inTag = true;
          this._tagName = best;
        } else {
          // 保留可能的不完整开标签（如 "<Moo" 或 "<mood "）
          let hold = 0;
          ...
          for (const tag of STRIP_TAGS) {
            ...
            // 也匹配 <tag 后面可能跟空格的情况 (e.g. "<mood ")
            const spaceOpen = `<${tag}`;
            if (lower.endsWith(spaceOpen) && spaceOpen.length > hold) hold = spaceOpen.length;
            const spaceOpen2 = `<${tag} `;
            if (lower.endsWith(spaceOpen2) && spaceOpen2.length > hold) hold = spaceOpen2.length;
          }
...
export class BridgeManager {
  ...
  _takePendingBatch(sessionKey) {
    ...
    const batch = {
      ...
      bridgeAudience: pending.bridgeAudience,
  ...
  /** 注入平台事件（用于 OneBot 反向 HTTP 等被动接入场景） */
  ingestPlatformEvent(platform, payload, agentId) {
    const entry = this._platforms.get(this._getPlatformKey(platform, agentId));
    if (!entry?.adapter) {
      return { ok: false, error: `${platform} not started` };
    }
    if (typeof entry.adapter.ingestEvent !== "function") {
      return { ok: false, error: `${platform} adapter does not support external event ingest` };
    }
    entry.adapter.ingestEvent(payload);
    return { ok: true };
  }
  ...
  async _handleMessage(platform, msg) {
    ...
    const audience = this._resolveAudience(platform, { userId, chatId, senderName, agentId, isOwner });
    const canOperate = !isGroup && audience.policy.allowWorkspaceActions === true;
    const bridgeAudience = this._toAudiencePayload(audience, { isGroup, canOperate });
    ...
    if (dispatcher && text && text.trim().startsWith("/") && canOperate) {
    ...
    if (rcState && !isGroup && rcState.isPending(sessionKey) && text && canOperate) {
      const sendReply = (t) => this._sendAdapterReply(entry.adapter, chatId, t, replyContext).catch(() => {});
    ...
    // ── 私聊：debounce + abort ──
    const line = !canOperate && senderName
      ? `${senderName}: ${text}`
      : text;

    let pending = this._pending.get(sessionKey);
    if (!pending) {
      pending = { kind: "dm-buffer", lines: [], attachments: [], platform, chatId, senderName, avatarUrl, userId, qqPrincipal, isGroup, isOwner, bridgeAudience, agentId, messageThreadId, replyContext };
      this._pending.set(sessionKey, pending);
    }
    pending.lines.push(line);
    this._appendPendingAttachments(entry, pending.attachments, attachments);
    Object.assign(pending, { platform, chatId, senderName, avatarUrl, userId, qqPrincipal, isGroup, isOwner, bridgeAudience, messageThreadId, replyContext });
  ...
  _createStreamDelivery({ adapter, chatId, isGroup, platform, messageThreadId, replyContext }) {
    ...
    if (mode === "batch_pulse") {
      return this._createBatchPulseDelivery({ adapter, chatId, capability, context });
    }
  ...
  _resolveStreamingCapability(adapter, isGroup) {
    ...
    if (capability.mode === "batch_pulse" && adapter?.sendReply) return capability;
    return null;
  }
  ...
  /**
   * batch_pulse 模式：流式期间定期发送简短脉冲提示当前进展，
   * 流结束后发送完整回复。适合不支持消息编辑的平台（如 QQ），
   * 避免逐行发送导致消息碎片化。
   *
   * 脉冲消息会根据已累积的文本内容动态生成，展示当前工作状态，
   * 而非千篇一律的固定文案。
   */
  _createBatchPulseDelivery({ adapter, chatId, capability, context }) {
    const pulseIntervalMs = Number.isFinite(capability?.pulseIntervalMs) ? capability.pulseIntervalMs : 20_000;
    let pulseTimer = null;
    let finished = false;
    let accumulated = "";

    // ── 脉冲文案生成 ──
    // 优先展示模型实际生成的内容片段，仅在无实质内容时回退到状态提示
    const _generatePulseText = () => {
      const text = accumulated;

      // 剥离内部标签，提取纯对话内容
      let cleaned = text;
      cleaned = cleaned.replace(/<(?:mood|pulse|reflect|think|thinking|tool_code)>[\s\S]*?<\/(?:mood|pulse|reflect|think|thinking|tool_code)>\s*/gi, "");
      cleaned = cleaned.replace(/```(?:mood|pulse|reflect)[\s\S]*?```\n*/gi, "");
      cleaned = cleaned.replace(/<(?:mood|pulse|reflect|think|thinking|tool_code)>[\s\S]*$/gi, "");
      cleaned = cleaned.trim();

      // thinking 块尚未闭合 → 仍在思考
      if (/<(think|thinking)>/i.test(text) && !/<\/(think|thinking)>/i.test(text)) {
        return "（正在思考…）";
      }

      // 有实质内容时，提取最后一条有意义的行作为进展展示
      ...
      // 内容特征检测（回退到状态提示）
      const hasCode = text.includes("```") || /`[^`]+`/.test(text);
      ...
      if (hasCode) {
        ...
        if (openFences % 2 !== 0) return "（正在写代码…）";
      }
      if (hasCode && hasTable) return "（正在整理代码和表格…）";
      if (hasCode && hasHeading) return "（正在编写代码说明…）";
      if (hasTable) return "（正在制作表格…）";
      if (hasHeading && hasList) return "（正在梳理要点…）";
      if (hasHeading) return "（正在组织内容…）";
      if (hasList) return "（正在逐条说明…）";

      const len = cleaned.length;
      if (len > 1500) return "（内容有点长，快写完了…）";
      if (len > 800) return "（写得有点多，请稍等…）";
      ...
      return hints[Math.floor(Date.now() / pulseIntervalMs) % hints.length];
    };

    const startPulse = () => {
      if (pulseTimer || finished) return;
      pulseTimer = setInterval(async () => {
        if (finished) { clearPulse(); return; }
        try {
          const label = _generatePulseText();
          await this._sendAdapterReply(adapter, chatId, label, context);
        } catch (err) {
          log.warn(`batch_pulse send failed: ${err.message}`);
        }
      }, pulseIntervalMs);
    };
    ...
    return {
      ...
      onDelta: (delta) => {
        accumulated += delta;
        if (!pulseStarted) { pulseStarted = true; startPulse(); }
      },
      finish: async (cleaned) => {
        finished = true;
        clearPulse();
        const { text: textOnly, mediaUrls } = splitMediaFromOutput(cleaned);
        if (textOnly.trim()) {
          await this._sendAdapterReply(adapter, chatId, textOnly.trim(), context);
        }
        return mediaUrls;
      },
    };
  ...
  _cleanStreamSnapshot(text) {
    ...
    for (const tag of STRIP_TAGS) {
      const open = new RegExp(`<${tag}\\s*>[\\s\\S]*$`, "i");
  ...
  async _flushPending(sessionKey) {
    ...
    // 取出所有缓冲消息和附件
    const { lines, attachments: pendingAttachments = [], platform, chatId, senderName, avatarUrl, userId, qqPrincipal, isGroup, isOwner, bridgeAudience, agentId, messageThreadId, replyContext } = batch;
    const canOperate = !isGroup && bridgeAudience?.allowWorkspaceActions === true;
    ...
    try {
      ...
      const rcState = this.engine.rcState;
      const rcAttachment = !isGroup && canOperate ? rcState?.getAttachment(sessionKey) : null;
      if (rcAttachment) {
      ...
      let reply = await this._hub.send(merged, {
        sessionKey,
        agentId,
        role: isGroup ? "guest" : canOperate ? "owner" : "guest",
        meta,
        isGroup,
        bridgeAudience,
  ...
  _resolveAudience(platform, { userId = null, chatId = null, senderName = null, agentId = null, isOwner = false } = {}) {
    const agent = agentId ? this.engine.getAgent(agentId) : null;
    if (!agent) {
      const relation = isOwner ? "self" : "stranger";
      return {
        matched: isOwner,
        relation,
        policy: getEffectiveBridgeRelationPolicy(relation),
        candidate: { platform, userId, chatId, name: senderName },
        source: isOwner ? "owner_policy" : "default",
      };
    }
    try {
      return resolveBridgeAudience(agent, {
        platform,
        userId,
        chatId,
        name: senderName,
  ...
  _toAudiencePayload(audience, { isGroup = false, canOperate = false } = {}) {
    const relation = audience?.relation || "stranger";
    const policy = audience?.policy || getEffectiveBridgeRelationPolicy(relation);
    return {
      relation,
      contactId: audience?.contact?.id || null,
      contactName: audience?.contact?.displayName || null,
      source: audience?.source || "default",
      hostPermissionMode: isGroup ? "social_readonly" : policy.hostPermissionMode,
      infoDisclosure: policy.infoDisclosure,
      toneProfile: policy.toneProfile,
      audiencePrompt: typeof policy.audiencePrompt === "string" && policy.audiencePrompt.trim()
        ? policy.audiencePrompt.trim()
        : null,
      promptSource: isGroup ? "public" : (policy.promptSource || "public"),
      allowWorkspaceActions: canOperate,
    };
  }
  ...
  _cleanReplyForPlatform(text) {
    ...
    cleaned = cleaned.replace(/```(?:mood|pulse|reflect)[\s\S]*?```\n*/gi, "");
    cleaned = cleaned.replace(/<(?:mood|pulse|reflect)\s*>[\s\S]*?<\/(?:mood|pulse|reflect)\s*>\s*/gi, "");
    // thinking 标签
    cleaned = cleaned.replace(/<(?:think|thinking)\s*>[\s\S]*?<\/(?:think|thinking)\s*>\s*/gi, "");
    // <tool_code> 标签
    cleaned = cleaned.replace(/<tool_code\s*>[\s\S]*?<\/tool_code\s*>\s*/gi, "");
    return cleaned.trim();
  }
