...
DEFAULT_GRAPH_PATH = "OH-Works/ontology/graph.jsonl"
DEFAULT_SCHEMA_PATH = "OH-Works/ontology/schema.yaml"
...
def create_entity(type_name: str, properties: dict, graph_path: str, entity_id: str = None, provenance: dict = None) -> dict:
    """Create a new entity."""
    ...
    if provenance:
        # Create Provenance entity first
        prov_id = provenance.get("id") or generate_id("Provenance")
        prov_entity = {
            "id": prov_id,
            "type": "Provenance",
            "properties": {k: v for k, v in provenance.items() if k != "id"},
            "created": timestamp,
            "updated": timestamp,
            "queried_count": 0,
        }
        prov_record = {"op": "create", "entity": prov_entity, "timestamp": timestamp}
        append_op(graph_path, prov_record)
        entity["provenance_id"] = prov_id
        
        # Also create derived_from relation
        rel_record = {
            "op": "relate",
            "from": entity_id,
            "rel": "derived_from",
        ...
        append_op(graph_path, rel_record)
...
def get_entity(entity_id: str, graph_path: str, track_usage: bool = True) -> dict | None:
    """Get entity by ID. Optionally track last_queried and queried_count."""
    entities, _ = load_graph(graph_path)
    entity = entities.get(entity_id)
    if entity and track_usage:
        touch_entity(entity_id, graph_path)
    return entity


def query_entities(type_name: str, where: dict, graph_path: str, track_usage: bool = True) -> list:
    """Query entities by type and properties. Optionally track last_queried and queried_count."""
    entities, _ = load_graph(graph_path)
    results = []
    ...
    if track_usage and results:
        for entity in results:
            touch_entity(entity["id"], graph_path)
...
def touch_entity(entity_id: str, graph_path: str) -> None:
    """Record an access event: set last_queried and increment queried_count."""
    entities, _ = load_graph(graph_path)
    if entity_id not in entities:
        return
    entity = entities[entity_id]
    timestamp = datetime.now(timezone.utc).isoformat()
    new_count = entity.get("queried_count", 0) + 1
    record = {
        "op": "update",
        "id": entity_id,
        "properties": {
...
def search_entities(keyword: str, graph_path: str, type_name: str = None) -> list:
    """Search entities by keyword across all string properties."""
    entities, _ = load_graph(graph_path)
    ...
    for entity in entities.values():
        if type_name and entity["type"] != type_name:
            continue
        # Search in id, type, and all property values
        searchable = [entity["id"], entity["type"]]
        for v in entity.get("properties", {}).values():
            if isinstance(v, str):
                searchable.append(v)
            elif isinstance(v, list):
                searchable.extend(str(item) for item in v)
            else:
                searchable.append(str(v) if v is not None else "")
...
def main():
    ...
    create_p.add_argument("--provenance", help="Provenance JSON: {source_type, source_uri, source_title?, section?, passage?, confidence?}")
    ...
    # Touch (mark entity as accessed)
    touch_p = subparsers.add_parser("touch", help="Record access to entity (updates last_queried/queried_count)")
    touch_p.add_argument("--id", required=True, help="Entity ID")
    touch_p.add_argument("--graph", "-g", default=DEFAULT_GRAPH_PATH)

    # Search
    search_p = subparsers.add_parser("search", help="Search entities by keyword")
    search_p.add_argument("--keyword", "-k", required=True, help="Search keyword")
    search_p.add_argument("--type", "-t", help="Filter by entity type")
    search_p.add_argument("--graph", "-g", default=DEFAULT_GRAPH_PATH)
    ...
    if args.command == "create":
        ...
        entity = create_entity(args.type, props, args.graph, args.id, provenance=provenance)
        ...
        touch_entity(args.id, args.graph)
        print(f"Touched: {args.id}")

    elif args.command == "search":
        results = search_entities(args.keyword, args.graph, type_name=args.type)
        print(json.dumps(results, indent=2))
