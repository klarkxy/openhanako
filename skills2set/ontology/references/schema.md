...
Provenance:
  ...
  properties:
    source_type: enum(document, conversation, web, manual, inference, skill_output)
    source_uri: string          # file path, URL, conversation ID, etc.
    source_title: string?       # human-readable title of the source
    section: string?            # section/chapter/heading name
    passage: string?            # exact passage or excerpt that produced this fact
    line_range: string?         # e.g. "L45-L67" or "p.12-p.14"
    confidence: enum(high, medium, low)?
    extracted_at: datetime?     # when this provenance was recorded
    context: string?            # surrounding context that gives the fact its meaning
    extractor: string?          # skill or agent that extracted this fact
```
...
## Entity-Level Built-in Fields

Every entity automatically carries these fields in addition to its type-specific properties:

```yaml
builtin_fields:
  provenance_id: ref(Provenance)?   # Where this fact came from
  last_queried: datetime?           # When this entity was last accessed via query/get
  queried_count: integer?           # How many times this entity has been queried (default 0)
```

These are managed by the ontology tool automatically:
- `provenance_id` is set at creation time via `--provenance` flag
- `last_queried` and `queried_count` are updated by `get`/`query`/`search` commands and the `touch` command
...
```yaml
derived_from:
  from_types: [Any]           # Any entity can be derived from a Provenance
  to_types: [Provenance]
  cardinality: many_to_one
  description: "Links an entity to its provenance record — where the knowledge came from"

supports:
  from_types: [Provenance]
  to_types: [Any]
  cardinality: one_to_many
  description: "Inverse of derived_from: a source supports multiple derived facts"

based_on:
  from_types: [Any]
  to_types: [Any]
  cardinality: many_to_many
  description: "One fact is based on another; for chaining inferences"
```
