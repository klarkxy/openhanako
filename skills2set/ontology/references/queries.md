...
```bash
# Search all entities for a keyword
python3 scripts/ontology.py search --keyword "meeting"

# Search within a specific type
python3 scripts/ontology.py search --keyword "Alice" --type Person
```

### Touch (Mark as Accessed)

```bash
# Manually record access to an entity
python3 scripts/ontology.py touch --id task_001
```

Note: `get` and `query` commands automatically update `last_queried` and `queried_count`. Use `touch` for explicit access tracking outside those commands.
...
# Load the graph
entities, relations = load_graph("OH-Works/ontology/graph.jsonl")

# Query entities
open_tasks = query_entities("Task", {"status": "open"}, "OH-Works/ontology/graph.jsonl")

# Get related
project_tasks = get_related("proj_001", "has_task", "OH-Works/ontology/graph.jsonl")
```
